...
public class BitbucketCommitParser implements CommitParserService {
    ...
    public List<CommitResponse> parseCommits(String jsonResponse) {
        ...
        try {
            JsonNode rootNode = objectMapper.readTree(jsonResponse).path("values");
    ...
    public CommitResponse parseCommitDetails(String jsonResponse) {
        ...
        try {
            ...
            String sha = commitNode.path("hash").asText();
            ...
            if (commitNode.has("files")) {
                ...
                for (JsonNode fileNode : commitNode.get("files")) {
                    String fileName = fileNode.path("path").asText();
                    int additions = fileNode.path("lines_added").asInt();
                    int deletions = fileNode.path("lines_removed").asInt();
